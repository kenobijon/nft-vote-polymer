...
if (!source || !destination) {
    console.error("Usage: node _deploy-nft-config.js <source_network> <destination_network>");
    process.exit(1);
}

// Function to run the deploy script and capture output
function deployAndCapture(srcNetwork, dstNetwork) {
    const allowedNetworks = getWhitelistedNetworks();
    if (!allowedNetworks.includes(srcNetwork) || !allowedNetworks.includes(dstNetwork)) {
        console.error("Invalid network. Please provide a valid network as an argument.");
        return;
    }
    exec(`npx hardhat run scripts/deployBallot.js --network ${srcNetwork}`, (error, stdout, stderr) => {
        if (error) {
      ...
      `);
            ...
            updateConfigDeploy(network, address, true);
    ...
    exec(`npx hardhat run scripts/deployNFTVote.js --network ${dstNetwork}`, (error, stdout, stderr) => {
        if (error) {
            console.error(`exec error: ${error}`);
            return;
        } else {
      ...
      `);
            ...
            // Update the config.json file
            updateConfigDeploy(network, address, false);
            console.log(`🆗 Updated ${process.env.CONFIG_PATH || "config.json"} with address ${address} on network ${network}`);
        } else {
            console.error("❌ Could not find contract address and network in output");
        }
...
function main() {
    deployAndCapture(source, destination);
}
